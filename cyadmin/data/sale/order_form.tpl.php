        ...
        <div id="tab-customer" class="vtabs-content">
          <table class="form">
            ...
            <tr>
              ...
              <td><input type="text" name="fax" value="<?php echo $mobile;?>" /></td>
            </tr>
          </table>
        </div>
          ...
          <table class="form">
            <tr>
              ...
              <td><select name="payment_address">
                  ...
                  <option value="<?php echo $address['address_id'];?>">{ <?php echo $address['username'];?> . ', ' . <?php echo $address['address_1'];?> . ', ' . <?php echo $address['city'];?> . ', ' . <?php echo $address['country'];?>}</option>
                 <?php } ?>
                </select></td>
            </tr>
            <tr>
              <td><span class="required">*</span> <?php echo $entry_username;?></td>
              <td><input type="text" name="payment_username" value="<?php echo $payment_username;?>" />
                <?php if($error_payment_username) { ?>
                <span class="error"><?php echo $error_payment_username;?></span>
               <?php } ?></td>
            </tr>
          ...
          <table class="form">
            <tr>
              ...
              <td><select name="shipping_address">
                  <option value="0" selected="selected"><?php echo $text_none;?></option>
                  <?php foreach((array)$addresses as $address) {?>
                  <option value="<?php echo $address['address_id'];?>"><?php echo $address['username']  . ', ' . $address['address'] . ', ' . $address['postcode'] . ', ' . $address['mobile']?></option>
                  <?php } ?>
                </select></td>
            </tr>
            <tr>
              <td><span class="required">*</span> <?php echo $entry_username;?></td>
              <td><input type="text" name="shipping_username" value="<?php echo $shipping_username;?>" />
                <?php if($error_shipping_username) { ?>
                <span class="error"><?php echo $error_shipping_username;?></span>
                <?php } ?></td>
            </tr>
            ...
            <tr>
              <td><?php echo $entry_address;?></td>
              <td><input type="text" name="shipping_address" value="<?php echo $shipping_address;?>" /></td>
            </tr>
            <!-- <tr>
              <td><span class="required">*</span> <?php echo $entry_city;?></td>
        ...
        <div id="tab-product" class="vtabs-content">
          <table class="list">
            ...
            <tbody id="product">
              ...
              <?php foreach((array)$order_products as $order_product) {?>
              <tr id="product-row<?php echo $product_row;?>">
                ...
                <td class="left"><?php echo $order_product['name'];?><br />
                  ...
                  <?php foreach((array)$order_product['option'] as $option) {?>
                  ...
                  <?php foreach((array)$order_product['download'] as $download) {?>
          ...
          <table class="list">
            ...
            <tfoot>
              <tr>
                <td class="left">&nbsp;</td>
                <td class="left"><a href="javascript:;" id="button-product" class="button"><?php echo $button_add_product;?></a></td>
              </tr>
            </tfoot>
          </table>
        </div>
        <div id="tab-voucher" class="vtabs-content">
          <table class="list">
            ...
            <tbody id="voucher">
              ...
              <?php foreach((array)$order_vouchers as $order_voucher) {?>
          ...
          <table class="list">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="left"><select name="voucher_theme_id">
                    <?php foreach((array)$voucher_themes as $voucher_theme) {?>
            ...
            <tfoot>
              <tr>
                <td class="left">&nbsp;</td>
                <td class="left"><a  href="javascript:;" id="button-voucher" class="button"><?php echo $button_add_voucher;?></a></td>
              </tr>
            </tfoot>
          </table>
        </div>
        <div id="tab-total" class="vtabs-content">
          <table class="list">
            ...
            <tbody id="total">
             ...
             <?php } ?>
              <?php foreach((array)$order_totals as $order_total) {?>
          ...
          <table class="list">
            ...
            <tfoot>
              <tr>
                <td class="left">&nbsp;</td>
                <td class="left"><a href="javascript:;" id="button-update" class="button"><?php echo $button_update_total;?></a></td>
              </tr>
            </tfoot>
          </table>
        </div>
...
$('input[name=\'customer\']').catcomplete({
	...
	select: function(event, ui) { 
		$('input[name=\'customer\']').prop('value', ui.item['label']);
		$('input[name=\'customer_id\']').prop('value', ui.item['value']);
		...
		for (i in  ui.item['address']) {
			html += '<option value="' + ui.item['address'][i]['address_id'] + '">'  + ui.item['address'][i]['username'] + ', ' + ui.item['address'][i]['address_1'] + ', ' + ui.item['address'][i]['city'] + ', ' + ui.item['address'][i]['country'] + '</option>';
		}
		...
		$('select[id=\'customer_group_id\']').prop('value', ui.item['customer_group_id']);
...
$('select[id=\'customer_group_id\']').live('change', function() {
	$('input[name=\'customer_group_id\']').prop('value', this.value);
	...
	$.ajax({
		...
		success: function(json) {
			if (json != '') {	
				...
				$('input[name=\'payment_username\']').prop('value', json['username']);
				$('input[name=\'payment_company\']').prop('value', json['company']);
				$('input[name=\'payment_company_id\']').prop('value', json['company_id']);
				$('input[name=\'payment_tax_id\']').prop('value', json['tax_id']);
	...
	$.ajax({
		...
		success: function(json) {
			if (json != '') {	
				...
				$('input[name=\'shipping_username\']').prop('value', json['username']);
				$('input[name=\'shipping_company\']').prop('value', json['company']);
				$('input[name=\'shipping_address\']').prop('value', json['address']);
					...
					new AjaxUpload('#button-option-' + option['product_option_id'], {
						...
						onComplete: function(file, json) {
							...
							if (json['success']) {
								...
								$('input[name=\'option[' + this._settings.data['product_option_id'] + ']\']').prop('value', json['file']);
							}
...
$('#button-product, #button-voucher, #button-update').live('click', function() {	
	...
	$.ajax({
		url: '<?php echo $store_url;?>index.php?route=checkout/manual&token=<?php echo $token;?>',//url前面加上了<?php echo $store_url;?>
		type: 'post',
		data: $(data),
		dataType: 'json',	
		beforeSend: function() {
		...
		success: function(json) {
			...
			if (json['error']) {
				...
				if (json['error']['username']) {
					$('input[name=\'username\']').after('<span class="error">' + json['error']['username'] + '</span>');
				}	
				...
				if (json['error']['payment']) {	
					...
					if (json['error']['payment']['username']) {
						$('input[name=\'payment_username\']').after('<span class="error">' + json['error']['payment']['username'] + '</span>');
					}	
				...
				if (json['error']['shipping']) {		
					...
					if (json['error']['shipping']['username']) {
						$('input[name=\'shipping_username\']').after('<span class="error">' + json['error']['shipping']['username'] + '</span>');
					}	
