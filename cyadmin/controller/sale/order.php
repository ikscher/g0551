...
class ControllerSaleOrder extends Controller {
	...
	//删除
  	...
  	private function getList() {
		...
		$this->data['refresh'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] , 'SSL');
    	...
    	foreach ($results as $result) {
			...
			if ($result['date_added']> strtotime('-' . (int)$this->config->get('config_order_edit') . ' day')) {
				$action[] = array(
			...
			$this->data['orders'][] = array(
				...
				'date_modified' => date('Y-m-d', $result['date_modified']),
    	...
    	$this->data['column_customer'] = $this->language->get('column_member');
		...
		$this->template = 'sale/order_list.html';
		/* $this->children = array(
			'common/header',
			'common/footer'
		); */
  	...
  	public function getForm() {
		...
		$this->data['entry_customer'] = $this->language->get('entry_member');
		...
		$this->data['tab_customer'] = $this->language->get('tab_member');
		...
		$this->data['tab_total'] = $this->language->get('tab_total');
 		...
 		if (isset($this->error['payment_username'])) {
			$this->data['error_payment_username'] = $this->error['payment_username'];
		} else {
			$this->data['error_payment_username'] = '';
		}

				
		if (isset($this->error['payment_address'])) {
			$this->data['error_payment_address'] = $this->error['payment_address'];
		} else {
			$this->data['error_payment_address'] = '';
		}
		...
		if (isset($this->error['payment_method'])) {
 		...
 		if (isset($this->error['shipping_username'])) {
			$this->data['error_shipping_username'] = $this->error['shipping_username'];
		} else {
			$this->data['error_shipping_username'] = '';
		}
		...
		if (isset($this->request->post['affiliate'])) {
			...
			$this->data['affiliate'] = ($order_info['affiliate_id'] ? $order_info['affiliate_username']: '');
		} else {
      		$this->data['affiliate'] = '';
    	}
    	...
    	if (isset($this->request->post['payment_username'])) {
      		$this->data['payment_username'] = $this->request->post['payment_username'];
    	} elseif (!empty($order_info)) { 
			$this->data['payment_username'] = $order_info['payment_username'];
		} else {
      		$this->data['payment_username'] = '';
    	}
    	...
    	if (isset($this->request->post['shipping_lastname'])) {
      		$this->data['shipping_username'] = $this->request->post['shipping_username'];
    	} elseif (!empty($order_info)) { 
			$this->data['shipping_username'] = $order_info['shipping_username'];
		} else {
      		$this->data['shipping_username'] = '';
    	}
		...
		$this->template = 'sale/order_form.html';
		/* $this->children = array(
			'common/header',
			'common/footer'
		); */
  	...
  	private function validateForm() {
    	...
    	if ((utf8_strlen($this->request->post['username']) < 2) || (utf8_strlen($this->request->post['username']) > 12)) {
    	...
    	/* if ((utf8_strlen($this->request->post['payment_username']) < 1) || (utf8_strlen($this->request->post['payment_username']) > 32)) {
      		$this->error['payment_username'] = $this->language->get('error_username');
    	}
		...
		if ($shipping) {
			if ((utf8_strlen($this->request->post['shipping_username']) < 2) || (utf8_strlen($this->request->post['shipping_username']) > 32)) {
				$this->error['shipping_username'] = $this->language->get('error_username');
			}
	...
	public function info() {
		...
		if ($order_info) {
            ...
            $this->load->model('sale/customer');
			$result=$this->model_sale_customer->getCustomerInfo($order_info['customer_id']);
			$customer_group_id=$result['customer_group_id'];
			$this->data['store_url']=$result['url'];
			$this->data['store_name']=$result['name'];
			...
			$this->data['affiliate_username'] = $order_info['affiliate_username'];
			...
			$this->data['commission'] = number_format($order_info['commission'],2,'.',',');
			...
			$this->template = 'sale/order_info.html';
			/* $this->children = array(
				'common/header',
				'common/footer'
			); */
			...
			$this->template = 'error/not_found.html';
			$this->children = array(
				'common/header',
				'common/footer'
			);
	...
	public function history() {
		...
		$this->template = 'sale/order_history.html';		
	...
	public function upload() {
  	...
  	public function invoice() {
		...
		$this->template = 'sale/order_invoice.html';
