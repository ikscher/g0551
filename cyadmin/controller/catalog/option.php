...
class ControllerCatalogOption extends Controller {
	...
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->redirect($this->url->link('catalog/option/update', 'token=' . $this->session->data['token'] . $url, 'SSL')); */
		}
	...
	private function getList() {
		...
		$this->data['refresh'] = $this->url->link('catalog/option', 'token=' . $this->session->data['token'] , 'SSL');
		...
		$this->template = 'catalog/option_list.html';
		/* $this->children = array(
			'common/header',
			'common/footer'
		); */
	...
	private function getForm() {
		...
		$this->template = 'catalog/option_form.html';
		/* $this->children = array(
			'common/header',
			'common/footer'
		); */
	...
	private function validateForm() {
		...
		$option_description=$this->request->post['option_description'];
		if ((utf8_strlen($option_description) < 1) || (utf8_strlen($option_description) > 128)) {
        ...
        // var_dump($this->request->post['option_value']);
		if (isset($this->request->post['option_value'])) {
			foreach ($this->request->post['option_value'] as $option_value_id => $option_value) {
				$option_value_description=$option_value['option_value_description']['name'];
				if ((utf8_strlen($option_value_description) < 1) || (utf8_strlen($option_value_description) > 128)) {
	...
	//http://test.cy0551.com/cy/cyadmin/index.php?route=catalog/option/autocomplete&token=c3e6c92eea120c7f2fbc6f633b8f3847&attribute_group_id=4&filter_name=c
	    ...
	    if( isset($this->request->get['attribute_group_id'])){
			...
			foreach ($options as $option) {
				...
				if ($option['type'] == 'select' || $option['type'] == 'radio' || $option['type'] == 'checkbox' || $option['type'] == 'image') {
					$option_values = $this->model_catalog_option->getOptionValues($option['option_id'],$attribute_group_id);
