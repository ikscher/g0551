...
class ModelSaleVoucher extends Model {
	...
	public function getVouchers($data = array()) {
		$sql = "SELECT v.voucher_id, v.code, v.from_name, v.from_email, v.to_name, v.to_email, (SELECT vtd.name FROM " . DB_PREFIX . "voucher_theme_description vtd WHERE vtd.voucher_theme_id = v.voucher_theme_id ) AS theme, v.amount, v.status, v.date_added FROM " . DB_PREFIX . "voucher v";
	...
	public function sendVoucher($voucher_id) {
		...
		if ($voucher_info) {
			...
			if ($order_info) {
				...
				$mail->setHtml($template->fetch('mail/voucher.html'));				
				$mail->send();
	...
	public function getVoucherHistories($voucher_id, $start = 0, $limit = 10) {
		...
		$query = $this->db->query("SELECT vh.order_id, username AS customer, vh.amount, vh.date_added FROM " . DB_PREFIX . "voucher_history vh LEFT JOIN `" . DB_PREFIX . "order` o ON (vh.order_id = o.order_id) WHERE vh.voucher_id = '" . (int)$voucher_id . "' ORDER BY vh.date_added ASC LIMIT " . (int)$start . "," . (int)$limit);
