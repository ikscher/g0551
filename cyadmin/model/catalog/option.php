...
class ModelCatalogOption extends Model {
	public function addOption($data) {
		...
		//foreach ($data['option_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "option_description SET option_id = '" . (int)$option_id  . "', name = '" . $this->db->escape($data['option_description']) . "'");
		//}
		...
		if (isset($data['option_value'])) {
			foreach ($data['option_value'] as $option_value) {
				...
				$this->db->query("INSERT INTO " . DB_PREFIX . "option_value_description SET option_value_id = '" . (int)$option_value_id . "', option_id = '" . (int)$option_id . "', name = '" . $this->db->escape($option_value_description['name']) . "'");
				//}
			}
		}
	}
	...
	public function getOptions($data = array()) {
		//$sql = "SELECT o.option_id,o.type,o.sort_order,od.name FROM `" . DB_PREFIX . "option` o LEFT JOIN " . DB_PREFIX . "option_description od ON (o.option_id = od.option_id) ";
		$sql = "SELECT o.option_id,o.type,o.sort_order,od.name FROM `" . DB_PREFIX . "option` o inner JOIN " . DB_PREFIX . "option_description od ON (o.option_id = od.option_id) ";
		
		/* if (isset($data['filter_name']) && !is_null($data['filter_name'])) {
			$sql .= " AND LCASE(od.name) LIKE '%" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "%'";
		} */
		
		if(isset($data['attribute_group_id']) && !is_null($data['attribute_group_id'])){
		    $sql .=" inner join ".DB_PREFIX."attribute_group ag on o.option_id=ag.option_id  where ag.attribute_group_id={$data['attribute_group_id']} ";
		}
	...
	public function getOptionValues($option_id,$attribute_group_id='') {
		...
		//产品所在的属性组  model(getAttribute)
		/* if(isset($product_id)){
		    $this->load->model('catalog/product');
			$this->load->model('catalog/category');
			$categoryId=$this->model_catalog_product->getProductCategories($product_id);//产品属于的分类（包括子类和父类）
		...
		if (!empty($attribute_group)){
		    $sql.=" AND ovd.attribute_group_id in ({$attribute_group}) ";
		}
		*/
	...
	public function getOptionValueDescriptions($option_id) {
		...
		foreach ($option_value_query->rows as $option_value) {
			...
			foreach ($option_value_description_query->rows as $option_value_description) {
				$option_value_description_data = array('name' => $option_value_description['name']);
			}
