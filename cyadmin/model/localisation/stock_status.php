...
class ModelLocalisationStockStatus extends Model {
	public function addStockStatus($data) {
		//foreach ($data['stock_status'] as $language_id => $value) {
			if (isset($stock_status_id)) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "stock_status SET stock_status_id = '" . (int)$stock_status_id . "', name = '" . $this->db->escape($data) . "'");
			} else {
				$this->db->query("INSERT INTO " . DB_PREFIX . "stock_status SET  name = '" . $this->db->escape($data) . "'");
	...
	public function getStockStatusDescriptions($stock_status_id) {
		...
		$result=$query->row;
		//$stock_status_data = array('name' => $result['name']);
