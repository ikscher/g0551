...
class ModelLocalisationReturnReason extends Model {
	...
	public function editReturnReason($return_reason_id, $data) {
		...
		foreach ($data['return_reason'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "return_reason SET return_reason_id = '" . (int)$return_reason_id  . "', name = '" . $this->db->escape($value['name']) . "'");
		}
	...
	public function getReturnReasons($data = array()) {
      	if ($data) {
			$sql = "SELECT * FROM " . DB_PREFIX . "return_reason ";
			...
			$return_reason_data = $this->cache->get('return_reason' );
		
			if (!$return_reason_data) {
				$query = $this->db->query("SELECT return_reason_id, name FROM " . DB_PREFIX . "return_reason ORDER BY name");
	
				$return_reason_data = $query->rows;
			
				$this->cache->set('return_reason', $return_reason_data);
			}	
	...
	public function getReturnReasonDescriptions($return_reason_id) {
		...
		$query = $this->db->query("SELECT name FROM " . DB_PREFIX . "return_reason WHERE return_reason_id = '" . (int)$return_reason_id . "'");
		
		/* foreach ($query->rows as $result) {
			$return_reason_data = array('name' => $result['name']);
		} */
	...
	public function getTotalReturnReasons() {
      	$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "return_reason ");
