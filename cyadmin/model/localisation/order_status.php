...
class ModelLocalisationOrderStatus extends Model {
	public function addOrderStatus($data) {
		...
		//foreach ($data['order_status'] as $language_id => $value) {
			if (isset($order_status_id)) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_status SET order_status_id = '" . (int)$order_status_id  . "', name = '" . $this->db->escape($value['name']) . "'");
			} else {
	...
	public function editOrderStatus($order_status_id, $data) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "order_status WHERE order_status_id = '" . (int)$order_status_id . "' limit 1");
	...
	public function getOrderStatusDescriptions($order_status_id) {
		...
		//foreach ($query->rows as $result) {
			$order_status_data = array('name' => $result['name']);
		//}
