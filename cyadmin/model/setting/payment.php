...
class ModelSettingPayment extends Model {
	...
	public function addStorePayment($data) {
		...
		$trade_type=$this->db->escape($data['alipay_trade_type']);
		$partner=$this->db->escape($data['alipay_partner']);
		$security_code=$this->db->escape($data['alipay_security_code']);
		$seller_email=$this->db->escape($data['alipay_seller_email']);
		$order_status_id=$this->db->escape($data['alipay_order_status_id']);
