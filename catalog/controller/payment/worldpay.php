...
class ControllerPaymentWorldPay extends Controller {
	protected function index() {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/worldpay.html')) {
			$this->template = $this->config->get('config_template') . '/template/payment/worldpay.html';
		} else {
			$this->template = 'default/template/payment/worldpay.html';
		}	
	...
	public function callback() {
		...
		if (isset($this->request->post['transStatus']) && $this->request->post['transStatus'] == 'Y') { 
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/worldpay_success.html')) {
				$this->template = $this->config->get('config_template') . '/template/payment/worldpay_success.html';
			} else {
				$this->template = 'default/template/payment/worldpay_success.html';
			}	
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/worldpay_failure.html')) {
				$this->template = $this->config->get('config_template') . '/template/payment/worldpay_failure.html';
			} else {
				$this->template = 'default/template/payment/worldpay_failure.html';
			}
