...
class ControllerModuleStore extends Controller {
	protected function index() {
		...
		if ($status) {
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/store.html')) {
				$this->template = $this->config->get('config_template') . '/template/module/store.html';
			} else {
				$this->template = 'default/template/module/store.html';
			}
