    	...
    	if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			$products = $this->cart->getProducts();
      		...
      		foreach ($products as $product) {
				...
				if ($product['minimum'] > $product_total) { //产品的下单数量小于产品的要求的最低订单数量
			...
			*/
			...
			if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
				...
				foreach ($results as $key => $value) {
					$sort_order[$key] = $this->config->get($value['code'] . '_sort_order');//配置显示统计排序的
				}
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/cart.html')) {
				$this->template = $this->config->get('config_template') . '/template/checkout/cart.html';
			} else {
				$this->template = 'default/template/checkout/cart.html';
			}
		...
		if ($product_info ) {
			...
			if (!$json) {
				...
				if ($product_info['image']) {
					$cartimage = $this->model_tool_image->resize($product_info['image'], 50, 50);                    
				} else {
					$cartimage = false;
				}       
					
				$json['cartimage']=$cartimage;
