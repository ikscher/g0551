		...
		if ($this->user->isLogged() && $this->user->hasPermission('modify', 'sale/order')) {	
			...
			if ($this->cart->hasShipping()) {		
				...
				if (!isset($json['error']['shipping'])) {
					...
					$address_data = array(
						'username'      => $this->request->post['shipping_username'],
						...
						'telphone'      =>$this->request->post['shipping_telphone'],
						'mobile'      =>$this->request->post['shipping_mobile'],
			...
			foreach ($results as $result) {
				if ($this->config->get($result['code'] . '_status')) {
					...
					$this->{'model_total_' . $result['code']}->getTotal($json['order_total'], $total);
				}
			...
			if (!isset($json['error']['payment'])) {
				...
				$address_data = array(
					'username'       => $this->request->post['payment_username'],
					'address'        => $this->request->post['payment_address'],
					// 'address_1'      => $this->request->post['payment_address_1'],
					'mobile'      => $this->request->post['payment_mobile'],
					'postcode'       => $this->request->post['payment_postcode'],
					'telphone'       => $this->request->post['payment_telphone'],
