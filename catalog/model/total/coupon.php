...
class ModelTotalCoupon extends Model {
	public function getTotal(&$total_data, &$total,$flag=false) { //&$total_data, &$total, &$taxes
		if (isset($this->session->data['coupon'])) {
			...
			$code=$this->session->data['coupon'];//优惠券代码
			$coupon_info = $this->model_checkout_coupon->getCoupon($code);
			
			if ($coupon_info) {
				$discount_total = 0;
				
				if (!$coupon_info['product']) {//产品不能使用优惠券
				...
				foreach ($this->cart->getProducts() as $product) {
					...
					if ($status) {
						if ($coupon_info['type'] == 'F') {//按金额
							$discount = $coupon_info['discount'] * ($product['total'] / $sub_total);
						} elseif ($coupon_info['type'] == 'P') { //按百分比
				...
				$total_data[] = array(
	    			...
	    			'text'       =>number_format(-$discount_total,2,'.',',') , //$this->currency->format(-$discount_total)
