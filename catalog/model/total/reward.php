...
class ModelTotalReward extends Model {
		...
		if (isset($this->session->data['reward'])) {
			...
			if ($this->session->data['reward'] <= $points) {
				...
				$total_data[] = array(
	    			...
	    			'text'       => number_format(-$discount_total),
	...
	public function confirm($order_info, $order_total) {
		...
		if ($points) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "customer_reward SET customer_id = '" . (int)$order_info['customer_id'] . "', description = '" . $this->db->escape(sprintf($this->language->get('text_order_id'), (int)$order_info['order_id'])) . "', points = '" . (float)-$points . "', date_added ={$time}");				
		}
	}		
}
?>