...
class ModelMerchantsOrderCreate extends Model {
	...
	*/
	public function getOrderList($data){
		...
		$order_id=isset($data['order_id'])?$this->db->escape($data['order_id']):"";
		...
		$ctelphone=isset($data['ctelphone'])?$this->db->escape($data['ctelphone']):"";
		...
		if($order_id!="") $where.=" And o.order_id='{$order_id}'";
		...
		if($ctelphone!="") $where.=" And o.telphone='{$ctelphone}'";
		//if($order_time!="") $where.=" And from_unixtime(a.date_added) like '{$order_time}%'";
		$res=array();
		
		$sql="select o.order_id,o.customer_id,o.username,o.mobile,o.total,o.order_status_id,o.date_added,c.email from ".DB_PREFIX."`order` o inner join ".DB_PREFIX."customer c on o.store_id={$store_id} and o.customer_id=c.customer_id ".$where;
	...
	*/
	public function getOrderOnce($intOrderId){
		...
		$query=$this->db->query("select a.order_id,a.customer_id,a.orderid,a.username,a.mobile,a.telphone,a.postcode,a.address,a.total,a.comment,a.order_status_id,a.date_added,b.email,b.username as user_username,b.nickname,b.mobile as user_mobile,b.telphone as user_telphone from `order` as a,customer as b where a.store_id={$store_id} and a.customer_id=b.customer_id and a.order_id={$intOrderId}");
		if($query->num_rows>0){
	   		return $query->row;
		}
		return $res;
