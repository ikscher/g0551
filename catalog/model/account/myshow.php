<?php
class ModelAccountMyshow extends Model {	
	...
	*/
	public function getContents($data=array()){
	    ...
	    $sql="select content_id,imageUrl,`favoriate`,`share`,`title`,`content`,`present`,`createtime`,`isShow`,customer_id from `". DB_PREFIX ."contents` ";
	...
	*/
	public function editContent($id,$data){
		...
		if(strpos($image,'cache')!==false){//已生产缓存文件，不更新数据库
		    ...
		    $image=preg_replace('/\/?\w*\/image\//','',$image);
	...
	*/
	public function addContent($data){
		...
		$sql="insert  into `". DB_PREFIX ."contents`(`title`,`content`,`present`,`createtime`,`isShow`,`imageUrl`,`customer_id`) values('{$title}','{$content}','{$present}','{$createtime}','{$isShow}','{$image}','{$customer_id}')";
