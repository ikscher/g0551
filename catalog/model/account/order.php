...
class ModelAccountOrder extends Model {
	public function getOrder($order_id) {
		$order_query = $this->db->query("SELECT o.*,s.name  as store_name FROM `" . DB_PREFIX . "order` o left join ".DB_PREFIX."store s on o.store_id=s.store_id  WHERE o.order_id = '{$order_id}' AND o.customer_id = '" . (int)$this->customer->getId() . "' AND o.order_status_id > '0'");
	...
	public function getOrderTotals($order_id,$code='') {
	    if(!empty($code)){
			$sql="SELECT order_total_id,order_id,code,title,text,value,sort_order FROM " . DB_PREFIX . "order_total WHERE order_id = '{$order_id}' and code='{$code}' ORDER BY sort_order";
			$query = $this->db->query($sql);
		    return $query->row;
		}else{
		    $sql="SELECT order_total_id,order_id,code,title,text,value,sort_order FROM " . DB_PREFIX . "order_total WHERE order_id = '{$order_id}' ORDER BY sort_order";
			$query = $this->db->query($sql);
			return $query->rows;
        }
	}	
	...
	public function getOrderHistories($order_id) {
		$query = $this->db->query("SELECT date_added, os.name AS status, oh.comment, oh.notify FROM " . DB_PREFIX . "order_history oh LEFT JOIN " . DB_PREFIX . "order_status os ON oh.order_status_id = os.order_status_id WHERE oh.order_id = '{$order_id}' ORDER BY oh.date_added");// AND oh.notify = '1' 通知才显示了
	...
	public function getTotalOrderVouchersByOrderId($order_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order_voucher` WHERE order_id =  '{$order_id}'");
