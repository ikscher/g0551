...
class ModelAccountReturn extends Model {
	...
	public function getReturns($start = 0, $limit = 20) {
		...
		$query = $this->db->query("SELECT r.return_id, r.order_id, r.username, rs.name as status, r.date_added FROM `" . DB_PREFIX . "return` r LEFT JOIN " . DB_PREFIX . "return_status rs ON (r.return_status_id = rs.return_status_id) WHERE r.customer_id = '" . $this->customer->getId()  . "' ORDER BY r.return_id DESC LIMIT " . (int)$start . "," . (int)$limit);
