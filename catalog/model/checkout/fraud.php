...
class ModelCheckoutFraud extends Model {
	public function getFraudScore($data) {
		...
		if ($fraud_info) {
			...
			http://www.uplook.cn/biancheng/123/1230506/
			paypal api
	...
	public function getFraud($order_id) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_fraud` WHERE order_id =  '{$order_id}'");
