...
class Pagination {
	...
	/**
	*  $results 结果集的css的class样式名称
	*  $links   链接页面css的class样式名称
	*/
	public function __construct($results,$links){
	...
	public function render() {
		...
		if ($num_pages > 1) {
   		...
   		if ($page < $num_pages) {
			$output .= ' <a href="' . str_replace('{page}', $page + 1, $this->url) . '"  title="'.$this->title_next.'">' . $this->text_next . '</a> <a href="' . str_replace('{page}', $num_pages, $this->url) . '">' . $this->text_last . '</a> ';
		}
